...
IDE: Replit
Summary: Take an input from the user for how many times to loop. Output on every loop where we are in the loop followed by "Hello"
*/
...
int main() {
  int input = 0;
  do {
    string promptString = "Please enter the number of times you would like to loop (>0): ";
  ...
  for (int x = 1; x <= input; x++) {
    ...
    if (x == 11 || x == 12 || x == 13 ||
        x % 100 == 11 || x % 100 == 12 || x % 100 == 13) {
      ordinal = to_string(x) + "th";
    // Then the rest is just checking if it needs to be a st, nd, rd, else it is a th.
    } else if (x % 10 == 1) {
